...
public class GenExam {
        ...
        public void initialize () {
            ...
            questions = utility.loadRandomQuestions("src/fx/db/Questions.json");


            if (!questions.isEmpty()) {
                pane.setPageCount(70);
        ...
        public void submit (javafx.event.ActionEvent event) throws Exception {
            ...
            double percentage = (score * 100.0) / 70;
            String mostAnsweredMaterial = getMostAnsweredMaterial();

            Score resultData = new Score(score, percentage, mostAnsweredMaterial);

            List<Score> results = utility.loadResults();
            results.add(resultData);

            utility.saveResults(results);
            Main.getInstance().setRoot("/fx/fxmlFiles/result.fxml");
        }
